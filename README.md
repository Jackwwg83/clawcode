# 🦞 ClawCode — Personal AI Assistant (Claude Agent SDK Edition)
...
**ClawCode** is a _personal AI assistant_ you run on your own devices, built with the **Claude Agent SDK**.
It answers you on the channels you already use (WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, iMessage, Microsoft Teams, WebChat), plus extension channels like BlueBubbles, Matrix, Zalo, and Zalo Personal. It can speak and listen on macOS/iOS/Android, and can render a live Canvas you control. The Gateway is just the control plane — the product is the assistant.

If you want a personal, single-user assistant that feels local, fast, and always-on, powered by Claude's official Agent SDK, this is it.

## About This Edition

**ClawCode** is the Claude Agent SDK edition of OpenClaw. This version replaces the original agent runtime with Anthropic's official **Claude Agent SDK**, providing:

- **Native Claude integration** — Built on Anthropic's official SDK for agent development
- **Enhanced capabilities** — Access to Claude's full agent features and tooling
- **Minimal changes** — All other OpenClaw systems (Gateway, channels, tools, etc.) remain intact
- **Same experience** — Keep all your channels, skills, and workflows

The primary difference is under the hood: the agent runtime now uses Claude Agent SDK instead of the Pi agent runtime. Everything else you know and love about OpenClaw works exactly the same.

### When to Use ClawCode vs OpenClaw

**Use ClawCode if:**
- You want to use Claude Agent SDK features and official Anthropic agent tooling
- You prefer Anthropic's official agent development framework
- You're building agent-specific extensions that leverage Claude Agent SDK

**Use OpenClaw if:**
- You want the most stable, community-supported version
- You need Pi agent runtime features
- You prefer the broader multi-model agent runtime flexibility

### Learn More

- [FEATURES.md](docs/FEATURES.md) — Full feature comparison and Claude Agent SDK capabilities
- [MIGRATION.md](docs/MIGRATION.md) — Migrating from OpenClaw to ClawCode
- [OpenClaw upstream](https://github.com/openclaw/openclaw) — Original project

[Website](https://openclaw.ai) · [Docs](https://docs.openclaw.ai) · [DeepWiki](https://deepwiki.com/openclaw/openclaw) · [Getting Started](https://docs.openclaw.ai/start/getting-started) · [Updating](https://docs.openclaw.ai/install/updating) · [Showcase](https://docs.openclaw.ai/start/showcase) · [FAQ](https://docs.openclaw.ai/start/faq) · [Wizard](https://docs.openclaw.ai/start/wizard) · [Nix](https://github.com/openclaw/nix-clawdbot) · [Docker](https://docs.openclaw.ai/install/docker) · [Discord](https://discord.gg/clawd)

Preferred setup: run the onboarding wizard (`clawcode onboard`). It walks through gateway, workspace, channels, and skills. The CLI wizard is the recommended path and works on **macOS, Linux, and Windows (via WSL2; strongly recommended)**.
...
clawcode gateway --port 18789 --verbose

# Send a message
clawcode message send --to +1234567890 --message "Hello from ClawCode"

# Talk to the assistant (optionally deliver back to any connected channel: WhatsApp/Telegram/Slack/Discord/Google Chat/Signal/iMessage/BlueBubbles/Microsoft Teams/Matrix/Zalo/Zalo Personal/WebChat)
clawcode agent --message "Ship checklist" --thinking high
```

Upgrading? [Updating guide](https://docs.openclaw.ai/install/updating) (and run `clawcode doctor`).
...
Switch channels (git + npm): `clawcode update --channel stable|beta|dev`.
...
```bash
git clone https://github.com/openclaw/clawcode.git
cd clawcode
...
pnpm clawcode onboard --install-daemon

# Dev loop (auto-reload on TS changes)
pnpm gateway:watch
```

Note: `pnpm clawcode ...` runs TypeScript directly (via `tsx`). `pnpm build` produces `dist/` for running via Node / the packaged `clawcode` binary.

## Security defaults (DM access)

ClawCode connects to real messaging surfaces. Treat inbound DMs as **untrusted input**.
...
- Approve with: `clawcode pairing approve <channel> <code>` (then the sender is added to a local allowlist store).
- Public inbound DMs require an explicit opt-in: set `dmPolicy="open"` and include `"*"` in the channel allowlist (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`).

Run `clawcode doctor` to surface risky/misconfigured DM policies.
...
- [Claude Agent SDK runtime](https://docs.openclaw.ai/concepts/agent) with native Claude integration, tool streaming and block streaming.
...
- [Browser control](https://docs.openclaw.ai/tools/browser): dedicated clawcode Chrome/Chromium, snapshots, actions, uploads, profiles.
...
- **[Browser control](https://docs.openclaw.ai/tools/browser)** — clawcode‑managed Chrome/Chromium with CDP control.
...
ClawCode can auto-configure Tailscale **Serve** (tailnet-only) or **Funnel** (public) while the Gateway stays bound to loopback. Configure `gateway.tailscale.mode`:
...
- `gateway.bind` must stay `loopback` when Serve/Funnel is enabled (ClawCode enforces this).
...
- Workspace root: `~/.clawcode/workspace` (configurable via `agents.defaults.workspace`).
- Injected prompt files: `AGENTS.md`, `SOUL.md`, `TOOLS.md`.
- Skills: `~/.clawcode/workspace/skills/<skill>/SKILL.md`.

## Configuration

Minimal `~/.clawcode/clawcode.json` (model + defaults):
...
- Link the device: `pnpm clawcode channels login` (stores creds in `~/.clawcode/credentials`).
...
ClawCode is based on **OpenClaw**, which was built for **Molty**, a space lobster AI assistant. 🦞
