...
1. **Existing config detection**
   - If `~/.clawcode/openclaw.json` exists, choose **Keep / Modify / Reset**.
...
2. **Model/Auth**
   ...
   - **OpenAI API key**: uses `OPENAI_API_KEY` if present or prompts for a key, then saves it to `~/.clawcode/.env` so launchd can read it.
...
- OAuth credentials live in `~/.clawcode/credentials/oauth.json`; auth profiles live in `~/.clawcode/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
- More detail: [/concepts/oauth](/concepts/oauth)

3. **Workspace**
   - Default `~/.clawcode/workspace` (configurable).
...
- Default workspaces follow `~/.clawcode/workspace-<agentId>`.
...
- Stores it under `~/.clawcode/tools/signal-cli/<version>/`.
...
Typical fields in `~/.clawcode/openclaw.json`:
...
WhatsApp credentials go under `~/.clawcode/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.clawcode/agents/<agentId>/sessions/`.
