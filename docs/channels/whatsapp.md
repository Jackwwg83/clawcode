...
2. Configure WhatsApp in `~/.clawcode/openclaw.json`.
...
- Credentials stored in `~/.clawcode/credentials/whatsapp/<accountId>/creds.json`.
- Backup copy at `creds.json.bak` (restored on corruption).
- Legacy compatibility: older installs stored Baileys files directly in `~/.clawcode/credentials/`.
