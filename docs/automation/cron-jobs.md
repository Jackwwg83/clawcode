...
- Jobs persist under `~/.clawcode/cron/` so restarts don’t lose schedules.
...
Cron jobs are persisted on the Gateway host at `~/.clawcode/cron/jobs.json` by default.
...
- Job store: `~/.clawcode/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.clawcode/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
