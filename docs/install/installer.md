...
This script installs `openclaw` into a prefix (default: `~/.clawcode`) and also installs a dedicated Node runtime under that prefix, so it can work on machines where you don’t want to touch the system Node/npm.
