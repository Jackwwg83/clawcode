...
```bash
rm -rf "${OPENCLAW_STATE_DIR:-$HOME/.clawcode}"
```
...
- If you used profiles (`--profile` / `OPENCLAW_PROFILE`), repeat step 3 for each state dir (defaults are `~/.clawcode-<profile>`).
...
Remove-Item -Force "$env:USERPROFILE\.clawcode\gateway.cmd"
```

If you used a profile, delete the matching task name and `~\.clawcode-<profile>\gateway.cmd`.
