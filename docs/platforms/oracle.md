...
- **Credential permissions:** `chmod 700 ~/.clawcode`
...
- `~/.clawcode/` — config, credentials, session data
- `~/.clawcode/workspace/` — workspace (SOUL.md, memory, artifacts)

Back up periodically:

```bash
tar -czvf openclaw-backup.tar.gz ~/.clawcode ~/.clawcode/workspace
```
