...
2. Copy `$OPENCLAW_STATE_DIR` (default: `~/.clawcode`) from the old machine.
3. Copy your workspace (default: `~/.clawcode/workspace`).
...
under `~/.clawcode/` (for example `~/.clawcode/agents/<agentId>/sessions/`).
...
(`~/.clawcode`) and workspace (`~/.clawcode/workspace`) stay untouched.
...
Use managed overrides instead of editing the repo copy. Put your changes in `~/.clawcode/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.clawcode/openclaw.json`). Precedence is `<workspace>/skills` > `~/.clawcode/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.clawcode/openclaw.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.clawcode/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which OpenClaw treats as `<workspace>/skills`.
...
3. Override the skill metadata (workspace or `~/.clawcode/skills`) to allow Linux:
   ```markdown
   ---
   name: imsg
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured OpenClaw workspace); OpenClaw treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.clawcode/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
...
- **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
  (`~/.clawcode` + your workspace directory).
...
Everything lives under `$OPENCLAW_STATE_DIR` (default: `~/.clawcode`):
...
Legacy single‑agent path: `~/.clawcode/agent/*` (migrated by `openclaw doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.clawcode/workspace`).

### Where should AGENTSmd SOULmd USERmd MEMORYmd live

These files live in the **agent workspace**, not `~/.clawcode`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or `memory.md`), `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.clawcode`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.clawcode/skills`).

Default workspace is `~/.clawcode/workspace`, configurable via:
...
Do **not** commit anything under `~/.clawcode` (credentials, sessions, tokens).
...
OpenClaw reads an optional **JSON5** config from `$OPENCLAW_CONFIG_PATH` (default: `~/.clawcode/openclaw.json`):

```
$OPENCLAW_CONFIG_PATH
```

If the file is missing, it uses safe‑ish defaults (including a default workspace of `~/.clawcode/workspace`).
...
- Daemons read env vars from `~/.clawcode/.env` (or the service environment).
...
- Restore from backup (git or a copied `~/.clawcode/openclaw.json`).
...
1. Put the missing keys in `~/.clawcode/.env` so they’re picked up even when the service doesn’t inherit your shell env.
...
1. Put the token in `~/.clawcode/.env`:
   ```
...
- If you used profiles (`--profile` / `OPENCLAW_PROFILE`), reset each state dir (defaults are `~/.clawcode-<profile>`).
...
- **Disk growth:** sessions + transcripts live under `~/.clawcode/agents/<agentId>/sessions/`.
...
- edit `agents.defaults.model` in `~/.clawcode/openclaw.json`
...
```
~/.clawcode/agents/<agentId>/agent/auth-profiles.json
```
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/.clawcode/agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/.clawcode/agent/*` (migrated by `openclaw doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.clawcode/.env` or enable `env.shellEnv`.
...
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.clawcode/.env` on the **gateway host**.
...
- Use `openclaw --profile <name> …` per instance (auto-creates `~/.clawcode-<name>`).
...
- macOS: `$OPENCLAW_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.clawcode/logs/...`; profiles use `~/.clawcode-<profile>/logs/...`)
