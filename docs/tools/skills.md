...
2. **Managed/local skills**: `~/.clawcode/skills`
3. **Workspace skills**: `<workspace>/skills`

If a skill name conflicts, precedence is:

`<workspace>/skills` (highest) → `~/.clawcode/skills` → bundled skills (lowest)

Additionally, you can configure extra skill folders (lowest precedence) via
`skills.load.extraDirs` in `~/.clawcode/openclaw.json`.
...
- **Shared skills** live in `~/.clawcode/skills` (managed/local) and are visible
...
- Download installs: `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`), `extract` (default: auto when archive detected), `stripComponents`, `targetDir` (default: `~/.clawcode/tools/<skillKey>`).

If no `metadata.openclaw` is present, the skill is always eligible (unless
disabled in config or blocked by `skills.allowBundled` for bundled skills).

## Config overrides (`~/.clawcode/openclaw.json`)
...
install (npm package or OpenClaw.app). `~/.clawcode/skills` exists for local
