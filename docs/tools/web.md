...
`~/.clawcode/openclaw.json` under `tools.web.search.apiKey`.

**Environment alternative:** set `BRAVE_API_KEY` in the Gateway process
environment. For a gateway install, put it in `~/.clawcode/.env` (or your
...
environment. For a gateway install, put it in `~/.clawcode/.env`.
