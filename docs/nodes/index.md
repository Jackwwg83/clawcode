...
- **Approvals**: enforced on the node host via `~/.clawcode/exec-approvals.json`.
...
- `--display-name` on `openclaw node run` / `openclaw node install` (persists in `~/.clawcode/node.json` on the node).
...
Approvals live on the node host at `~/.clawcode/exec-approvals.json`.
...
- On headless node host, `system.run` is gated by exec approvals (`~/.clawcode/exec-approvals.json`).
...
- The node host stores its node id, token, display name, and gateway connection info in `~/.clawcode/node.json`.
- Exec approvals are enforced locally via `~/.clawcode/exec-approvals.json`
