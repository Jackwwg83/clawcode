...
- Auth profiles (OAuth + API keys): `~/.clawcode/agents/<agentId>/agent/auth-profiles.json`
- Runtime cache (managed automatically; don’t edit): `~/.clawcode/agents/<agentId>/agent/auth.json`

Legacy import-only file (still supported, but not the main store):

- `~/.clawcode/credentials/oauth.json` (imported into `auth-profiles.json` on first use)
