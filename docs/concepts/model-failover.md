...
- Secrets live in `~/.clawcode/agents/<agentId>/agent/auth-profiles.json` (legacy: `~/.clawcode/agent/auth-profiles.json`).
- Config `auth.profiles` / `auth.order` are **metadata + routing only** (no secrets).
- Legacy import-only OAuth file: `~/.clawcode/credentials/oauth.json` (imported into `auth-profiles.json` on first use).
...
Profiles live in `~/.clawcode/agents/<agentId>/agent/auth-profiles.json` under `profiles`.
