...
read_when:
  - Designing workspace memory (~/.clawcode/workspace) beyond daily Markdown logs
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.clawcode/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.clawcode/workspace` as canonical human-readable memory.
...
```
~/.clawcode/workspace/.memory/index.sqlite
```
...
Pragmatic take for `~/.clawcode/workspace`:
