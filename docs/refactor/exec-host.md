...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.clawcode/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.clawcode/node.json` (node id + pairing token).
...
Path: `~/.clawcode/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.clawcode/exec-approvals.sock",
...
- Unix socket at `~/.clawcode/exec-approvals.sock` (0600).
