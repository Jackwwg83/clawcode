...
variables like `DISPLAY` should live in `~/.clawcode/.env` (loaded early by the
gateway).
...
Symptoms: `pwd` or file tools show `~/.clawcode/sandboxes/...` even though you
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${OPENCLAW_CONFIG_PATH:-$HOME/.clawcode/openclaw.json}"
```
...
```bash
ls -la ~/.clawcode/agents/<agentId>/sessions/
```
...
trash "${OPENCLAW_STATE_DIR:-$HOME/.clawcode}/credentials" # if logout can't cleanly remove everything
...
#   ${OPENCLAW_CONFIG_PATH:-$HOME/.clawcode/openclaw.json} -> { logging: { level: "trace" } }
#
...
| Gateway service logs (supervisor) | macOS: `$OPENCLAW_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.clawcode/logs/...`; profiles use `~/.clawcode-<profile>/logs/...`)<br />Linux: `journalctl --user -u openclaw-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "OpenClaw Gateway (<profile>)" /V /FO LIST` |
...
trash "${OPENCLAW_STATE_DIR:-$HOME/.clawcode}"
