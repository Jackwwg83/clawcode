---
summary: "All configuration options for ~/.clawcode/openclaw.json with examples"
read_when:
...
OpenClaw reads an optional **JSON5** config from `~/.clawcode/openclaw.json` (comments + trailing commas allowed).

If the file is missing, OpenClaw uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.clawcode/workspace`). You usually only need a config to:
...
use `config.patch` or `openclaw config set`. Keep a backup of `~/.clawcode/openclaw.json`.
...
openclaw gateway call config.apply --params '{
  "raw": "{\\n  agents: { defaults: { workspace: \\"~/.clawcode/workspace\\" } }\\n}\\n",
...
{
  agents: {
    defaults: { workspace: "~/.clawcode/workspace" },
    list: [
      {
        id: "main",
...
```json5
// ~/.clawcode/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.clawcode/workspace" }],
}
```
...
```json5
// ~/.clawcode/clients/mueller/agents.json5
[
...
```json5
// ~/.clawcode/clients/mueller/broadcast.json5
{
...
- a global fallback `.env` from `~/.clawcode/.env` (aka `$OPENCLAW_STATE_DIR/.env`)
...
- `<agentDir>/auth-profiles.json` (default: `~/.clawcode/agents/<agentId>/agent/auth-profiles.json`)

**Auth resolution priority:** When making API requests, OpenClaw resolves credentials in this order:

1. **Auth profiles** (`auth-profiles.json`) — preferred; CLI `--api-key`/`--token` flags write here
2. **Environment variables** (e.g., `ANTHROPIC_API_KEY`, `CRS_API_KEY`)
3. **`models.providers.*.apiKey`** in config — can be an env var name (e.g., `"CRS_API_KEY"`) or `${ENV_VAR}` syntax

To avoid plaintext keys in config, either omit `apiKey` and rely on auth profiles, or set `apiKey` to an
env var name.
...
- `~/.clawcode/credentials/oauth.json` (or `$OPENCLAW_STATE_DIR/credentials/oauth.json`)
...
- `~/.clawcode/agent/*` (migrated by `openclaw doctor` into `~/.clawcode/agents/<defaultAgentId>/agent/*`)
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.clawcode/credentials/whatsapp/biz
          // authDir: "~/.clawcode/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.clawcode/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.clawcode/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.clawcode/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.clawcode/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.clawcode/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.clawcode/workspace-home" },
      { id: "work", workspace: "~/.clawcode/workspace-work" },
    ],
  },
  bindings: [
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.clawcode/settings/tts.json",
      elevenlabs: {
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.clawcode/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.clawcode/sandboxes",
        docker: {
...
`~/.clawcode/agents/<agentId>/agent/models.json` or by defining the same schema inside your
...
**CLI shortcut:** `openclaw models providers add` generates `models.providers` config interactively (or non-interactively with flags). Use `--api-key` or `--token` for auth (mutually exclusive). Use `--headers '{"x-custom":"value"}'` to add custom headers, and `--auth-header` to enable auth header passthrough. See `openclaw models providers --help` for full options.

Auth follows the standard order: **auth profiles → env vars → `models.providers.*.apiKey`**. The CLI
stores `--api-key`/`--token` in auth profiles (preferred). If you want to avoid plaintext in config,
set `apiKey` to an env var name (or `${ENV_VAR}`), or omit `apiKey` and rely on auth profiles.

When `models.providers` is present, OpenClaw writes/merges a `models.json` into
`~/.clawcode/agents/<agentId>/agent/` on startup:
...
Third-party Anthropic-compatible endpoints should be configured as **custom providers**
(do not rely on `ANTHROPIC_BASE_URL` alone). Example:

```json5
{
  env: { CRS_API_KEY: "cr_..." },
  agents: {
    defaults: { model: { primary: "crs/claude-sonnet-4-5-20250929" } },
  },
  models: {
    mode: "merge",
    providers: {
...
- Override the agent config root with `OPENCLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.clawcode/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.clawcode/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.clawcode/agents/{agentId}/sessions/sessions.json",
...
overrides. Applies to **bundled** skills and `~/.clawcode/skills` (workspace skills
...
from `~/.clawcode/extensions`, `<workspace>/.openclaw/extensions`, plus any
...
- OpenClaw.app watches `~/.clawcode/openclaw.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.clawcode/openclaw.json` (or `OPENCLAW_CONFIG_PATH`) and applies changes automatically.
...
- `~/.clawcode/openclaw.json` (or `OPENCLAW_CONFIG_PATH`)
...
- `openclaw --dev …` → uses `~/.clawcode-dev` + shifts ports from base `19001`
- `openclaw --profile <name> …` → uses `~/.clawcode-<name>` (port via config/env/flags)
...
```bash
OPENCLAW_CONFIG_PATH=~/.clawcode/a.json \
OPENCLAW_STATE_DIR=~/.clawcode-a \
...
Default root: `~/.clawcode/workspace/canvas`  
...
{
  canvasHost: {
    root: "~/.clawcode/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.clawcode/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.clawcode/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.clawcode/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_openclaw-gw._tcp` under `~/.clawcode/dns/` using the configured discovery domain (example: `openclaw.internal.`).
