...
- serve your chosen domain (example: `openclaw.internal.`) from `~/.clawcode/dns/<domain>.db`
...
- Set `gateway.bind: "tailnet"` in `~/.clawcode/openclaw.json`.
...
- `gateway.bind` in `~/.clawcode/openclaw.json` controls the Gateway bind mode.
