...
- Rewrite `~/.clawcode/openclaw.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.clawcode/sessions/` to `~/.clawcode/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.clawcode/agent/` to `~/.clawcode/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.clawcode/credentials/*.json` (except `oauth.json`)
  - to `~/.clawcode/credentials/whatsapp/<accountId>/...` (default account id: `default`)
...
- **Multiple state dirs**: warns when multiple `~/.clawcode` folders exist across
  home directories or when `OPENCLAW_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.clawcode/openclaw.json` is
