...
- Config hot reload watches `~/.clawcode/openclaw.json` (or `OPENCLAW_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__openclaw__/canvas/` from `~/.clawcode/workspace/canvas`. Disable with `canvasHost.enabled=false` or `OPENCLAW_SKIP_CANVAS_HOST=1`.
...
- `OPENCLAW_STATE_DIR=~/.clawcode-dev`
- `OPENCLAW_CONFIG_PATH=~/.clawcode-dev/openclaw.json`
- `OPENCLAW_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.clawcode/workspace-dev` when you run `setup`/`onboard` under `--dev`.
...
```bash
OPENCLAW_CONFIG_PATH=~/.clawcode/a.json OPENCLAW_STATE_DIR=~/.clawcode-a openclaw gateway --port 19001
OPENCLAW_CONFIG_PATH=~/.clawcode/b.json OPENCLAW_STATE_DIR=~/.clawcode-b openclaw gateway --port 19002
```
