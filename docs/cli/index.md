...
- `--dev`: isolate state under `~/.clawcode-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.clawcode-<name>`.
...
- `--workspace <dir>`: agent workspace path (default `~/.clawcode/workspace`).
...
- `paste-token`: `--provider <name>`, `--profile-id <id>`, `--expires-in <duration>`, `--base-url <url>` (anthropic only; sets `env.ANTHROPIC_BASE_URL` + `env.ANTHROPIC_AUTH_TOKEN`, but does not override built-in `anthropic` base URL)

### `models providers add|list|remove`

Add, list, or remove custom model providers.
...
- `--input <types>`: Input types (`text`, `image`, `text,image`)
...
- `--auth-header`: Include auth header in requests
- `--set-default`: Set as default model

Note: `--api-key`, `--token`, and `--api-key-env` are mutually exclusive; use only one.

For third-party Anthropic-compatible endpoints, use `models providers add` to create a custom provider and set your default model to `provider/model`.
Auth follows the standard order: **auth profiles → env vars → `models.providers.*.apiKey`**. The
`--api-key`/`--token` flags store credentials in auth profiles (preferred). Use `--api-key-env` to
store an env var name directly in config without creating an auth profile.

Options for `list`: `--json`, `--plain`

`remove <id>`: Remove a custom provider by id.
