...
describe("state + config path candidates", () => {
  ...
  it("orders default config candidates: ~/.clawcode first, then legacy dirs", () => {
    const home = "/home/test";
    const candidates = resolveDefaultConfigCandidates({} as NodeJS.ProcessEnv, () => home);
    // New default dir (.clawcode) comes first, then legacy dirs in order
    const expected = [
      // ~/.clawcode (new default)
      path.join(home, ".clawcode", "openclaw.json"),
      path.join(home, ".clawcode", "clawdbot.json"),
      path.join(home, ".clawcode", "moltbot.json"),
      path.join(home, ".clawcode", "moldbot.json"),
      // Legacy dirs in order: .clawdbot, .moltbot, .moldbot, .openclaw
  ...
  it("prefers ~/.clawcode when it exists", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "clawcode-state-"));
    try {
      const newDir = path.join(root, ".clawcode");
  ...
  it("falls back to ~/.openclaw when ~/.clawcode missing but ~/.openclaw exists", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "clawcode-legacy-"));
    try {
      const legacyDir = path.join(root, ".openclaw");
      await fs.mkdir(legacyDir, { recursive: true });
      // .clawcode does NOT exist, .openclaw exists
      const resolved = resolveStateDir({} as NodeJS.ProcessEnv, () => root);
