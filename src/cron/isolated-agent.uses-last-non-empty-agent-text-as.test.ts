...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses last non-empty agent text as summary", async () => {
    await withTempHome(async (home) => {
      ...
      vi.mocked(runAgentViaSdk).mockResolvedValue({
        payloads: [{ text: "first" }, { text: " " }, { text: " last " }],
        meta: {
          durationMs: 5,
          agentMeta: { sessionId: "s", provider: "p", model: "m" },
  ...
  it("appends current time after the cron header line", async () => {
    await withTempHome(async (home) => {
      ...
      const call = vi.mocked(runAgentViaSdk).mock.calls.at(-1)?.[0] as {
        prompt?: string;
      };
  ...
  it("uses model override when provided", async () => {
    await withTempHome(async (home) => {
      ...
      expect(res.status).toBe("ok");
      const call = vi.mocked(runAgentViaSdk).mock.calls[0]?.[0] as {
        provider?: string;
        model?: string;
      };
  ...
  it("wraps external hook content by default", async () => {
    await withTempHome(async (home) => {
      ...
      expect(res.status).toBe("ok");
      const call = vi.mocked(runAgentViaSdk).mock.calls[0]?.[0] as { prompt?: string };
  ...
  it("defaults thinking to low for reasoning-capable models", async () => {
    await withTempHome(async (home) => {
      ...
      const callArgs = vi.mocked(runAgentViaSdk).mock.calls.at(-1)?.[0];
  ...
  it("passes mcpServers to runAgentViaSdk with correct sessionKey", async () => {
    await withTempHome(async (home) => {
      const storePath = await writeSessionStore(home);
      const deps: CliDeps = {
        sendMessageWhatsApp: vi.fn(),
      ...
      const serverNames = callArgs?.mcpServers?.map((s) => s.name) ?? [];
