...
import { runAgentViaSdk } from "../agent/run-agent-via-sdk.js";
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(
    async (home) => {
      ...
      vi.mocked(runAgentViaSdk).mockReset();
...
describe("queue followups", () => {
  ...
  it("collects queued messages and drains after run completes", async () => {
    ...
    await withTempHome(async (home) => {
      ...
      const collectImpl = async (params: { prompt: string }) => {
      ...
      vi.mocked(runEmbeddedPiAgent).mockImplementation(collectImpl as never);
      vi.mocked(runAgentViaSdk).mockImplementation(collectImpl as never);
      ...
      expect(runAgentViaSdk).not.toHaveBeenCalled();
      ...
      expect(runAgentViaSdk).toHaveBeenCalledTimes(2);
  ...
  it("summarizes dropped followups when cap is exceeded", async () => {
    await withTempHome(async (home) => {
      const prompts: string[] = [];
      const summarizeImpl = async (params: { prompt: string }) => {
        prompts.push(params.prompt);
        return makeResult("ok");
      };
      vi.mocked(runEmbeddedPiAgent).mockImplementation(summarizeImpl as never);
      vi.mocked(runAgentViaSdk).mockImplementation(summarizeImpl as never);
