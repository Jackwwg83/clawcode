...
describe("getReplyFromConfig media note plumbing", () => {
  it("includes all MediaPaths in the agent prompt", async () => {
    await withTempHome(async (home) => {
      ...
      const mockImpl = async (params: { prompt: string }) => {
        seenPrompt = params.prompt;
        return makeResult("ok");
      };
      vi.mocked(runEmbeddedPiAgent).mockImplementation(mockImpl as never);
      vi.mocked(runAgentViaSdk).mockImplementation(mockImpl as never);
