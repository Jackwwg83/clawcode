...
describe("agentCommand", () => {
  ...
  it("does not duplicate agent events from embedded runs", async () => {
    await withTempHome(async (home) => {
      ...
      vi.mocked(runAgentViaSdk).mockImplementationOnce(async (params) => {
  ...
  it("passes mcpServers to runAgentViaSdk", async () => {
    await withTempHome(async (home) => {
      ...
      await agentCommand({ message: "test mcp", to: "+1999" }, runtime);
      ...
      // Should have memory, sessions, message, nodes, browser, canvas servers
