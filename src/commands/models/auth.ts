...
export async function modelsAuthPasteTokenCommand(
  ...
  await updateConfig((cfg) => {
    let next = applyAuthProfileConfig(cfg, { profileId, provider, mode: "token" });

    // For anthropic with custom base URL, also set env vars so non-interactive gateway can use them
  ...
  runtime.log(`Auth profile: ${profileId} (${provider}/token)`);
  if (provider === "anthropic" && baseUrl) {
    runtime.log(`Config env: ANTHROPIC_BASE_URL=${baseUrl}`);
    runtime.log(`Config env: ANTHROPIC_AUTH_TOKEN=*** (set)`);
  }
}
