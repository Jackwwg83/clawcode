...
describe("runAgentViaSdk", () => {
  ...
  it("passes mcpServers to SDK runner options", async () => {
    const mockQuery = mockSdkRunner([
      { type: "complete", stopReason: "end_turn" },
    ]);

    const mcpServers = [
      { name: "memory", command: "node mcp-memory" },
      { name: "sessions", command: "node mcp-sessions" },
    ];

    await runAgentViaSdk({ ...baseParams, mcpServers });
  ...
  it("omits mcpServers from SDK options when not provided", async () => {
    ...
    const sdkOptions = mockQuery.mock.calls[0][1];
...
describe("resolveAllowedTools", () => {
  ...
  it("filters MCP tools via policy like builtin tools", () => {
    ...
    vi.mocked(resolveEffectiveToolPolicy).mockReturnValue({
      ...defaultEffectivePolicy,
      globalPolicy: { deny: ["memory_search"] }, // Use canonical name, not MCP name
    });
    vi.mocked(isToolAllowedByPolicyName).mockImplementation(
      (name: string, policy: any) => {
        if (!policy) return true;
...
describe("buildSdkSystemPrompt", () => {
  ...
  it("passes full context to createOpenClawCodingTools (matching attempt.ts)", async () => {
    const mockSandbox = {
      enabled: true,
    ...
    await buildSdkSystemPrompt({
      ...
      model: "claude-sonnet-4-5-20250514",
      channel: "telegram",
      agentAccountId: "acc123",
      sandbox: mockSandbox,
      bashElevated: { enabled: true },
      // Full context params
      messageTo: "telegram:user:123",
      messageThreadId: "456",
      groupId: "group1",
      groupChannel: "#general",
      groupSpace: "workspace1",
    ...
    expect(createOpenClawCodingTools).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        modelId: "claude-sonnet-4-5-20250514",
        exec: expect.objectContaining({ elevated: { enabled: true } }),
      }),
    );
  });

  it("restores skill env on error path (try/finally protection)", async () => {
    const restoreFn = vi.fn();
    vi.mocked(applySkillEnvOverrides).mockReturnValue(restoreFn);

    // Force buildEmbeddedSystemPrompt to throw an error
    vi.mocked(buildEmbeddedSystemPrompt).mockImplementation(() => {
      throw new Error("System prompt build failed");
    });
    ...
    // restoreSkillEnv should still be called even though an error occurred
    expect(restoreFn).toHaveBeenCalledTimes(1);
  });

  it("does not double-restore on success path", async () => {
    const restoreFn = vi.fn();
    vi.mocked(applySkillEnvOverrides).mockReturnValue(restoreFn);

    const { restoreSkillEnv } = await buildSdkSystemPrompt({
      workspaceDir: "/ws",
      allowedToolNames: ["Read"],
    });

    // On success, restoreFn should NOT be called inside buildSdkSystemPrompt
    expect(restoreFn).not.toHaveBeenCalled();

    // Caller should invoke it later
    restoreSkillEnv?.();
    expect(restoreFn).toHaveBeenCalledTimes(1);
  });
});
