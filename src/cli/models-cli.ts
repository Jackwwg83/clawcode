...
export function registerModelsCli(program: Command) {
  ...
  const providers = models.command("providers").description("Manage custom model providers");

  providers
    .command("add")
    .description("Add a custom model provider")
    .option("--id <id>", "Provider id (e.g. my-provider)")
    .option("--base-url <url>", "Base URL (e.g. https://api.example.com/v1)")
    .option(
      "--api <type>",
      "API type (anthropic-messages, openai-completions, openai-responses, google-generative-ai, bedrock-converse-stream)",
    )
    .option("--model <id>", "Model id (e.g. claude-3-opus)")
    .option("--model-name <name>", "Model display name (defaults to model id)")
    .option("--context-window <n>", "Context window size (default: 200000)")
    .option("--max-tokens <n>", "Max output tokens (default: 8192)")
    .option("--input <types>", "Input types: text, image, or text,image (default: text)")
    .option("--reasoning", "Model supports reasoning/thinking", false)
    .option("--api-key <key>", "API key (stored in auth profile)")
    .option("--token <token>", "Auth token (stored in auth profile)")
    .option("--api-key-env <name>", "Env var name for API key (stored in config, not auth profile)")
    .option("--headers <json>", "Custom headers as JSON (e.g. '{\"x-api-key\":\"abc\"}')")
    .option("--auth-header", "Include auth header in requests", false)
    .option("--set-default", "Set this model as the default", false)
    .action(async (opts) => {
      await runModelsCommand(async () => {
        await modelsProvidersAddCommand(
          {
            ...
            headers: opts.headers as string | undefined,
            authHeader: opts.authHeader ? true : undefined,
  ...
  providers
    ...
    .action(async (opts) => {
      await runModelsCommand(async () => {
        await modelsProvidersListCommand(
          {
            json: Boolean(opts.json),
            plain: Boolean(opts.plain),
          },
  ...
  providers
    ...
    .action(async (id: string) => {
      await runModelsCommand(async () => {
        await modelsProvidersRemoveCommand(id, defaultRuntime);
      });
    });
  ...
  auth
    ...
    .option(
      "--base-url <url>",
      "Custom base URL (anthropic only). Sets env.ANTHROPIC_BASE_URL + env.ANTHROPIC_AUTH_TOKEN.",
    )
    .action(async (opts) => {
      await runModelsCommand(async () => {
