...
import { buildMcpServers } from "../../agent/mcp-servers.js";
...
import { createDefaultGatewayAgentRunner } from "./agent-bridge-integration.js";
...
export const agentHandlers: GatewayRequestHandlers = {
  agent: async ({ params, respond, context }) => {
    ...
    // Use GatewayAgentRunner (Claude Agent SDK) instead of legacy agentCommand
    const workspaceDir =
      (cfgForAgent as { workspaceDir?: string } | undefined)?.workspaceDir ??
      (cfg as { workspaceDir?: string })?.workspaceDir ??
      process.cwd();

    (async () => {
      const runner = createDefaultGatewayAgentRunner();
      const payloads: Array<{ text?: string; isComplete?: boolean }> = [];

      // Construct mcpServers using helper
      ...
      for await (const payload of runner.run({
        message,
        sessionKey: requestedSessionKey ?? "agent:main:main",
        workspaceDir,
