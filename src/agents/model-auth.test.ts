...
describe("getApiKeyForModel", () => {
  ...
  it("resolves ANTHROPIC_AUTH_TOKEN for anthropic provider", async () => {
    ...
    try {
      ...
      // Set the AUTH_TOKEN (lowest priority)
      process.env.ANTHROPIC_AUTH_TOKEN = "test-auth-token-123";
      ...
      expect(resolved?.apiKey).toBe("test-auth-token-123");
      expect(resolved?.source).toContain("ANTHROPIC_AUTH_TOKEN");
    } finally {
      if (previousOauth === undefined) {
        delete process.env.ANTHROPIC_OAUTH_TOKEN;
      } else {
        process.env.ANTHROPIC_OAUTH_TOKEN = previousOauth;
      }
  ...
  it("prefers ANTHROPIC_API_KEY over ANTHROPIC_AUTH_TOKEN", async () => {
    ...
    try {
      delete process.env.ANTHROPIC_OAUTH_TOKEN;
      process.env.ANTHROPIC_API_KEY = "api-key-higher-priority";
      process.env.ANTHROPIC_AUTH_TOKEN = "auth-token-lower-priority";
      ...
      const resolved = resolveEnvApiKey("anthropic");
      expect(resolved).not.toBeNull();
      expect(resolved?.apiKey).toBe("api-key-higher-priority");
